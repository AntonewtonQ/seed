...
import { acceleratorNotifications } from "./accelerator-data"
...
export function AcceleratorNotificationsPage() {
  return (
    <>
      <AcceleratorPageHeader
        ...
        description="Alertas sobre bloqueios, data rooms, readiness e marcos dos programas."
        title="Notificações"
      />

      <div className="grid gap-4 px-4 py-6 md:px-8 md:py-8 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <section className="grid gap-3">
          {acceleratorNotifications.map((notification) => (
            <Link
        ...
        <aside className="grid content-start gap-4">
          <article className="rounded-lg bg-[color:var(--seed-petroleum)] p-5 text-[color:var(--seed-cream)] shadow-[0_18px_42px_rgba(15,61,62,0.2)]">
            ...
            <p className="mt-3 text-sm font-semibold leading-6 text-[color:var(--seed-tint)]">
              Cada alerta deve apontar para uma startup, programa ou bloqueio que precisa de ação.
            </p>
          </article>
        </aside>
      </div>
